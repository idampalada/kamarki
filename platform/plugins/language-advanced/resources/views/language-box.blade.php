@if (count($languages) > 1)
    ...
    <div id="list-others-language">
        @foreach($languages as $language)
            @if ($currentLanguage && $language->lang_code != $currentLanguage->lang_code)
                {!! language_flag($language->lang_flag, $language->lang_name) !!}
                <a href="{{ Route::has($route['edit']) ? Request::url() . ($language->lang_code != Language::getDefaultLocaleCode() ? '?ref_lang=' . $language->lang_code : null) : '#' }}" target="_blank" class="d-inline-block ms-1">{{ $language->lang_name }} <i class="fas fa-external-link-alt"></i></a>
                <br>
            @endif
        @endforeach
    </div>

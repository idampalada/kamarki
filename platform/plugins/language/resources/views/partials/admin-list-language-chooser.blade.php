        ...
        @foreach ($languages as $language)
            @if ($language->lang_code !== Language::getCurrentAdminLocaleCode())
                <span>
                    ...
                    <a href="{{ route($route, array_merge($params ?? [], $language->lang_code === Language::getDefaultLocaleCode() ? [] : ['ref_lang' => $language->lang_code])) }}" class="d-inline-block ms-1 vertical-align-t mt-1">{{ $language->lang_name }}</a>
                </span>&nbsp;
            @endif
        @endforeach
